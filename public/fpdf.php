...
class FPDF
{
	...
	function __construct($orientation = 'P', $unit = 'mm', $size = 'A4')
	{
		...
		// Core fonts
		$this->CoreFonts = array('courier', 'times', 'helvetica', 'symbol', 'zapfdingbats');
		// Scale factor
		if ($unit == 'pt')
			$this->k = 1;
		elseif ($unit == 'mm')
		...
		if (!isset($this->fonts[$fontkey])) {
			...
			if (in_array($family, $this->CoreFonts)) {
				if ($family == 'symbol' || $family == 'zapfdingbats')
					$style = '';
				$fontkey = $family . $style;
				// Do NOT call AddFont for core fonts!
				$this->fonts[$fontkey] = array(
					'type' => 'Core',
					'name' => ucfirst($family) . ($style ? '-' . $style : ''),
					'up' => -100,
					'ut' => 50,
	...
	/**
	 * Returns the character widths array for core fonts.
	 */
	protected function _getCoreFontWidths($family, $style)
	{
		// Minimal implementation for core fonts: Courier, Times, Helvetica, Symbol, ZapfDingbats
		$widths = array();

		switch ($family) {
			case 'courier':
				// All widths are 600 for Courier
				for ($i = 0; $i < 256; $i++) {
					$widths[chr($i)] = 600;
				}
				break;
			...
			case 'arial':
				...
				// Fill missing ASCII codes with 556 (average width)
				for ($i = 0; $i < 256; $i++) {
					$c = chr($i);
					if (!isset($widths[$c])) {
						$widths[$c] = 556;
			...
			case 'symbol':
			case 'zapfdingbats':
				// For symbol and zapfdingbats, set all widths to 600
				for ($i = 0; $i < 256; $i++) {
					$widths[chr($i)] = 600;
				}
				break;
			default:
				// Default to 600 for all characters
				for ($i = 0; $i < 256; $i++) {
					$widths[chr($i)] = 600;
				}
				break;
